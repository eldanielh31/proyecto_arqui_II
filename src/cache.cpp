...
Cache::Cache(PEId owner, Bus& bus, Memory& mem)
  ...
  // Ahora 'line_bytes_' y 'num_sets_' ya están inicializados (por orden de miembros).
...
bool Cache::handle_load_miss(Addr addr, std::size_t size, Word& out) {
  ...
  if (line.valid && line.dirty) {
    ...
    Word w; std::memcpy(&w, line.data.data(), sizeof(Word));
...
bool Cache::load(Addr addr, std::size_t size, Word& out) {
  ...
  if (way >= 0) return read_hit(set_idx, way, out);
