...
namespace sim
{
  ...
  Program Assembler::assemble_from_string(const std::string &src)
  {
    ...
    for (auto &l : lines)
    {
      if (!l.empty() && l.back() == ':')
      {
        ...
        if (lab.empty())
          throw std::runtime_error("Label vacío");
        if (label_to_pc.count(lab))
          throw std::runtime_error("Label duplicado: " + lab);
        label_to_pc[lab] = pc;
      }
      else
      {
    ...
    for (auto &l : code_only)
    {
      ...
      {
        if (tok.size() != 4)
          throw std::runtime_error("Sintaxis FMUL: FMUL Rd, Ra, Rb");
        ins.op = OpCode::FMUL;
        ins.rd = parse_reg(tok[1]);
        ins.ra = parse_reg(tok[2]);
      ...
      {
        if (tok.size() != 4)
          throw std::runtime_error("Sintaxis FADD: FADD Rd, Ra, Rb");
        ins.op = OpCode::FADD;
        ins.rd = parse_reg(tok[1]);
        ins.ra = parse_reg(tok[2]);
        ins.rb = parse_reg(tok[3]);
      ...
      {
        if (tok.size() != 2)
          throw std::runtime_error("Sintaxis INC: INC Reg");
        ins.op = OpCode::INC;
        ins.rd = parse_reg(tok[1]);
      }
      ...
      {
        if (tok.size() != 2)
          throw std::runtime_error("Sintaxis DEC: DEC Reg");
        ins.op = OpCode::DEC;
        ins.rd = parse_reg(tok[1]);
      }
      ...
      {
        if (tok.size() != 2)
          throw std::runtime_error("Sintaxis JNZ: JNZ label (usa REG0 implícito)");
        ins.op = OpCode::JNZ;
        ins.label = tok[1];
  ...
  Program Assembler::assemble_from_file(const std::string &path)
  {
    ...
    if (!in)
      throw std::runtime_error("No se puede abrir archivo ASM: " + path);
